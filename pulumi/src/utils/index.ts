...
export class Utils {
  ...
  public static normalizePulumiConfig(obj: unknown): Record<string, any> {
  ...
  public static parsePulumiConfigRaw(raw: unknown): Record<string, any> {
    if (typeof raw === 'string') {
      // Try vals first, then plain JSON
      try {
        const resolved = Utils.resolveVALS(raw);
  ...
  public static toWorkspaceConfig(raw: unknown): Record<string, any> {
    const parsed = Utils.parsePulumiConfigRaw(raw);
    return Utils.normalizePulumiConfig(parsed);
  }

  /** Get a config entry value supporting both plain string and { value, secret } objects. */
  public static getPulumiConfigValue(cfg: Record<string, any>, key: string): string | undefined {
    const v = cfg?.[key];
    if (v == null) return undefined;
    if (typeof v === 'string') return v;
    if (typeof v === 'object' && 'value' in (v as any)) {
      const val = (v as any).value;
      return typeof val === 'string' ? val : (() => { try { return JSON.stringify(val); } catch { return String(val); } })();
    }
    try { return JSON.stringify(v); } catch { return String(v); }
  }

  /** Extract minimal AWS details from Pulumi config map if present. */
  public static extractAwsFromPulumiConfig(cfg: Record<string, any>): { region?: string; profile?: string; sharedConfigFiles: string[] } | undefined {
    const region = Utils.getPulumiConfigValue(cfg, 'aws:region');
    const profile = Utils.getPulumiConfigValue(cfg, 'aws:profile');
  ...
  public static extractGcpFromPulumiConfig(cfg: Record<string, any>): { region?: string } | undefined {
    const region = Utils.getPulumiConfigValue(cfg, 'gcp:region');
