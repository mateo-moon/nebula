...
export class PrometheusOperator extends pulumi.ComponentResource {
  constructor(
    ...
    const lokiChartArgs: ChartArgs = {
      ...
      values: {
        ...
        singleBinary: {
          ...
          resources: (mergedValues as any).loki?.singleBinary?.resources || {
            requests: {
              cpu: "500m",
              memory: "1Gi"
            },
    ...
    // Create Prometheus remote_write auth secret and Ingress if configured
    if (args.prometheusRwAuthHtpasswd && args.prometheusRwHost) {
      new k8s.core.v1.Secret(
        "prometheus-rw-auth-secret",
        {
          metadata: {
            name: "prometheus-rw-auth",
            namespace: namespaceName,
          },
          stringData: {
            auth: args.prometheusRwAuthHtpasswd,
          },
        },
        { parent: this, dependsOn: [namespace] }
      );
        ...
        "prometheus-rw-ingress",
        {
          metadata: {
            ...
            annotations: {
              ...
              "external-dns.alpha.kubernetes.io/hostname": args.prometheusRwHost,
              "nginx.ingress.kubernetes.io/auth-type": "basic",
              "nginx.ingress.kubernetes.io/auth-secret": "prometheus-rw-auth",
              "nginx.ingress.kubernetes.io/auth-realm": "Authentication Required - Prometheus Remote Write",
            },
          },
          spec: {
            ...
            tls: [
              {
                secretName: "prometheus-rw-tls",
                hosts: [args.prometheusRwHost],
              },
            ],
            rules: [
              {
                host: args.prometheusRwHost,
                http: {
                  paths: [
                    {
                      path: "/api/v1/write",
