...
export interface GkeConfig {
  /** GCP project ID (passed internally from Gcp module via inherited provider) */
...
export class Gke extends pulumi.ComponentResource {
  ...
  constructor(
    ...
    // Project is passed from parent Gcp module (extracted from inherited provider)
    const gcpProject = pulumi.output(args.project);
    ...
    this.cluster = new gcp.container.Cluster(
      clusterName,
      {
        ...
        workloadIdentityConfig: { workloadPool: pulumi.interpolate`${gcpProject}.svc.id.goog` },
    ...
    const projectInfo = gcp.organizations.getProjectOutput({ projectId: gcpProject }, { parent: this });
    ...
    if (args.createSystemNodePool) {
      if (!nodeGroups['system']) {
        nodeGroups['system'] = {
          ...
          }, // Generic label for selecting system nodes
    ...
    this.kubeconfig = pulumi.all([
      ...
      // Include project name prefix in context if not already present
      let context = clusterName;
      if (pulumiProject && !clusterName.startsWith(`${pulumiProject}-`)) {
        context = `${pulumiProject}-${clusterName}`;
      }
