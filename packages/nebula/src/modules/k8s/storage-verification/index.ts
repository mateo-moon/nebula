/**
 * Declarative, continuously reconciled storage verification.
 *
 * `StorageVerificationSetup` installs an XRD and Composition. A
 * `StorageVerification` XR then drives a complete EBS lifecycle without an
 * operator-run script:
 *
 ...
 * provider-aws so the workflow can verify their type, encryption, and zone.
 *
 * Prerequisites:
 ...
 * poll interval when realtime mode is off), so an idle verification wakes up
 * and starts a new cycle once `intervalSeconds` has elapsed.
 */
...
export const STORAGE_VERIFICATION_API_GROUP = "nebula.io";
export const STORAGE_VERIFICATION_API_VERSION = `${STORAGE_VERIFICATION_API_GROUP}/v1alpha1`;
export const STORAGE_VERIFICATION_KIND = "XStorageVerification";
export const STORAGE_VERIFICATION_PLURAL = "xstorageverifications";
export const STORAGE_VERIFICATION_COMPOSITION = "storage-verification";
export const STORAGE_VERIFICATION_NAMESPACE = "storage-verification";

export interface StorageVerificationSetupConfig {
  /** Namespace where verification workloads run (default: storage-verification). */
  namespace?: string;
  ...
  /** Composition name (default: storage-verification). */
  compositionName?: string;
}

export interface StorageVerificationConfig {
  ...
  /** Namespace created by StorageVerificationSetup (default: storage-verification). */
  namespace?: string;
  ...
  expandedSize?: string;
  /** Data written to the expansion verification (default: 800 MiB). */
  fillMiB?: number;
...
export class StorageVerificationSetup extends Construct {
  ...
  constructor(
    scope: Construct,
    id: string,
    config: StorageVerificationSetupConfig = {},
  ) {
    super(scope, id);

    const namespace = config.namespace ?? STORAGE_VERIFICATION_NAMESPACE;
    ...
    const compositionName =
      config.compositionName ?? STORAGE_VERIFICATION_COMPOSITION;
  ...
  private createRbac(
    ...
    new ApiObject(this, "crossplane-workload-role", {
      ...
      kind: "Role",
      metadata: {
        name: "storage-verification-crossplane",
        namespace,
    ...
    // verification namespace. It never creates, updates, or deletes that claim.
    new ApiObject(this, "kubernetes-provider-pvc-reader", {
      apiVersion: "rbac.authorization.k8s.io/v1",
      kind: "Role",
      metadata: {
        name: "storage-verification-pvc-reader",
        namespace,
        annotations: syncWave(-9),
      },
      rules: [
    ...
    new ApiObject(this, "kubernetes-provider-pv-reader", {
      ...
      kind: "ClusterRole",
      metadata: {
        name: "storage-verification-pv-reader",
        annotations: syncWave(-9),
      },
      rules: [
        {
  ...
  private createXrd(
    ...
    return new CompositeResourceDefinitionV2(this, "xrd", {
      metadata: {
        name: `${STORAGE_VERIFICATION_PLURAL}.${STORAGE_VERIFICATION_API_GROUP}`,
        annotations: syncWave(-10),
      },
      spec: {
        group: STORAGE_VERIFICATION_API_GROUP,
        names: {
          kind: STORAGE_VERIFICATION_KIND,
          plural: STORAGE_VERIFICATION_PLURAL,
        },
  ...
  private createComposition(
    ...
    return new Composition(this, "composition", {
      metadata: {
        ...
        labels: {
          "crossplane.io/xrd": `${STORAGE_VERIFICATION_PLURAL}.${STORAGE_VERIFICATION_API_GROUP}`,
        },
      },
      spec: {
        compositeTypeRef: {
          apiVersion: STORAGE_VERIFICATION_API_VERSION,
          kind: STORAGE_VERIFICATION_KIND,
        },
        mode: CompositionSpecMode.PIPELINE,
        pipeline: [
          {
            step: "run-storage-verification",
            functionRef: { name: functionName },
            input: {
              apiVersion: "gotemplating.fn.crossplane.io/v1beta1",
              kind: "GoTemplate",
              source: "Inline",
              inline: { template: STORAGE_VERIFICATION_TEMPLATE },
            },
          },
        ],
      },
...
/** Creates one continuously-reconciled storage verification XR. */
export class StorageVerification extends Construct {
  public readonly xr: ApiObject;

  constructor(scope: Construct, id: string, config: StorageVerificationConfig) {
    super(scope, id);

    if (config.zones.length !== 3) {
      throw new Error("StorageVerification requires exactly three availability zones");
    }
    ...
    for (const [field, value, minimum] of [
      ...
      if (!Number.isInteger(value) || value < minimum) {
        throw new Error(
          `StorageVerification ${field} must be an integer greater than or equal to ${minimum}`,
        );
      }
    }
...
const STORAGE_VERIFICATION_TEMPLATE = String.raw`
...
{{- range $i, $zone := $spec.zones -}}
  ...
  {{- if and $deployment $deployment.resource -}}
    {{- $observedMode := default "" (index (default (dict) $deployment.resource.metadata.annotations) "nebula.io/storage-verification-mode") -}}
...
{{- if and $autoPvc $autoPvc.resource -}}
  ...
  {{- if and $autoDeployment $autoDeployment.resource -}}
    {{- $autoMode := default "" (index (default (dict) $autoDeployment.resource.metadata.annotations) "nebula.io/storage-verification-mode") -}}
...
{{- if and $restoreDeployment $restoreDeployment.resource -}}
  {{- $restoreMode := default "" (index (default (dict) $restoreDeployment.resource.metadata.annotations) "nebula.io/storage-verification-mode") -}}
...
{{/* The three durable verification volumes and their remounting Deployments. */}}
{{- if $keepBase -}}
---
apiVersion: v1
kind: ConfigMap
metadata:
  ...
  labels:
    app.kubernetes.io/name: storage-verification
...
data:
  marker: nebula-storage-verification-v1
{{- range $i, $zone := $spec.zones }}
{{- $pvcKey := printf "data-pvc-%d" $i }}
{{- $deploymentKey := printf "data-deployment-%d" $i }}
{{- $pvObserverKey := printf "data-pv-observer-%d" $i }}
...
metadata:
  ...
  annotations:
    ...
    nebula.io/storage-verification-mode: {{ $mode }}
spec:
  replicas: {{ $replicas }}
  strategy:
    type: Recreate
  selector:
    matchLabels:
      nebula.io/storage-verification: {{ $xr.metadata.name }}-data-{{ $i }}
  template:
    metadata:
      labels:
        nebula.io/storage-verification: {{ $xr.metadata.name }}-data-{{ $i }}
...
metadata:
  ...
  annotations:
    ...
    nebula.io/storage-verification-mode: fill
spec:
  replicas: 1
  strategy:
    type: Recreate
  selector:
    matchLabels:
      nebula.io/storage-verification: {{ $xr.metadata.name }}-autoresize
  template:
    metadata:
      labels:
        nebula.io/storage-verification: {{ $xr.metadata.name }}-autoresize
...
metadata:
  ...
  annotations:
    ...
    nebula.io/storage-verification-mode: restore
spec:
  replicas: 1
  strategy:
    type: Recreate
  selector:
    matchLabels:
      nebula.io/storage-verification: {{ $xr.metadata.name }}-restore
  template:
    metadata:
      labels:
        nebula.io/storage-verification: {{ $xr.metadata.name }}-restore
...
{{- $latestFailureMessage := default "The latest storage verification cycle failed" $lastFailureReason }}
---
...
conditions:
  - type: StorageVerificationPassed
    ...
    message: {{ if $latestResultSucceeded }}"The latest storage verification cycle succeeded"{{ else if $latestResultFailed }}{{ $latestFailureMessage | quote }}{{ else }}"No storage verification cycle has completed yet"{{ end }}
    target: Composite
`.trim();
