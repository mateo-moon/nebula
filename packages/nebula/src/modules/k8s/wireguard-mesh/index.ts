...
export class WireGuardMesh extends BaseConstruct<WireGuardMeshConfig> {
  constructor(scope: Construct, id: string, config: WireGuardMeshConfig) {
    ...
    const initScript = [
      ...
      'PEER_NAME=""',
      "# Read the lookup table via a heredoc (NOT a pipe) so both loops run in",
      "# the current shell — that lets us stop at the FIRST match instead of",
      "# letting later substring matches overwrite it (last-match-wins).",
      'while IFS="|" read -r patterns name; do',
      '  [ -n "$patterns" ] || continue',
      '  OLD_IFS="$IFS"; IFS=","',
      "  for pat in $patterns; do",
      '    [ -n "$pat" ] || continue',
      '    case "$NODE_NAME" in',
      '      *"$pat"*) PEER_NAME="$name"; break ;;',
      "    esac",
      "  done",
      '  IFS="$OLD_IFS"',
      '  if [ -n "$PEER_NAME" ]; then break; fi',
      "done <<WG_EOF",
      "$PEER_LOOKUP",
      "WG_EOF",
      "",
