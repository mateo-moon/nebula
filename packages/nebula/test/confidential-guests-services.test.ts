...
const primary = { app: "guests", role: "primary" }, maintenance = { app: "guests", role: "maintenance" };
...
test("ingress policies first, then Services, exactly as declared", () => {
  ...
  assert.deepEqual(render({
    namespace: NAMESPACE,
    ingress: [
      { name: "primary-control", podSelector: primary, ports: [9443], from: [maintenance] },
      { name: "maintenance-access", podSelector: maintenance, ports: [8080, 2200] },
    ],
    services: [
      { name: "guest-primary", selector: holder, ports: [9443, 9445], clusterIP: "192.0.2.10", publishNotReadyAddresses: true },
      { name: "guest-maintenance", selector: maintenance, ports: [8080], clusterIP: "2001:db8::10", publishNotReadyAddresses: false },
      { name: "guest-any", selector: maintenance, ports: [8080] },
    ],
  }), [
    { apiVersion: "networking.k8s.io/v1", kind: "NetworkPolicy", metadata: meta("primary-control"), spec: {
      ...
      ingress: [{ from: [{ podSelector: { matchLabels: maintenance } }], ports: [{ port: 9443, protocol: "TCP" }] }] } },
    { apiVersion: "networking.k8s.io/v1", kind: "NetworkPolicy", metadata: meta("maintenance-access"), spec: {
      podSelector: { matchLabels: maintenance }, policyTypes: ["Ingress"],
      ingress: [{ ports: [{ port: 8080, protocol: "TCP" }, { port: 2200, protocol: "TCP" }] }] } },
    { apiVersion: "v1", kind: "Service", metadata: meta("guest-primary"), spec: {
      ...
      ports: [{ name: "tcp-9443", port: 9443, targetPort: 9443, protocol: "TCP" }, { name: "tcp-9445", port: 9445, targetPort: 9445, protocol: "TCP" }] } },
    { apiVersion: "v1", kind: "Service", metadata: meta("guest-maintenance"), spec: {
      clusterIP: "2001:db8::10", selector: maintenance, publishNotReadyAddresses: false,
      ports: [{ name: "tcp-8080", port: 8080, targetPort: 8080, protocol: "TCP" }] } },
    { apiVersion: "v1", kind: "Service", metadata: meta("guest-any"), spec: {
      selector: maintenance, ports: [{ name: "tcp-8080", port: 8080, targetPort: 8080, protocol: "TCP" }] } },
  ]);
});
