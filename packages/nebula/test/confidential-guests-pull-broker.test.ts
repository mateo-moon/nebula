...
const node = sha256Hex("example node release"), maintenance = sha256Hex("example maintenance release");
...
const props = (change: Partial<AttestedPullBrokerProps> = {}): AttestedPullBrokerProps => ({
  ...
  initData: { form: "equals", value: node },
...
test("the 'equals' form renders byte-identically to the hand-written broker manifests", () => {
  const policy = policyFor(`ev.init_data == "${node}"`);
  assert.equal(pullBrokerPolicy(["default", "registry", "pull"], { form: "equals", value: node }), policy);
...
test("the 'in' form admits exactly the listed init-data hashes, in order, and can expose the pull secret as a resource", () => {
  const policy = policyFor(`ev.init_data in ["${maintenance}","${node}"]`);
  const initData = { form: "in", values: [maintenance, node] } as const;
...
test("init-data hashes are reviewed nonzero SHA-256 values", () => {
  const bad: unknown[] = ["", node.slice(1), node.toUpperCase(), "0".repeat(64), `${node}0`, 42, null];
  for (const value of bad) {
    assert.throws(() => render(props({ initData: { form: "equals", value } as any })), /init-data/, String(value));
    assert.throws(() => render(props({ initData: { form: "in", values: [node, value] } as any })), /init-data/, String(value));
  }
  ...
  assert.throws(() => render(props({ initData: { form: "equals", value: node, values: [node] } as any })), /init-data/);
});
...
test("names, selectors, images and commands are validated", () => {
  const refusals: [string, Partial<AttestedPullBrokerProps>, RegExp][] = [
    ...
    ["misspelt prop", { initdata: { form: "equals", value: node } } as any, /unknown field initdata/],
...
test("the broker publishes where guests reach it and the resource they request", () => {
  ...
  assert.equal(broker.policy, pullBrokerPolicy(["default", "registry", "pull"], { form: "equals", value: node }));
