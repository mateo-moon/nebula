...
def _extract_pending_tool_ids(result: dict) -> list[str]:
    """Extract pending tool-call IDs from an input-required A2A result.

    kagent's requireApproval sends adk_request_confirmation DataParts carrying
    the original function call ID. These live in status.message.parts AND history
    (NOT just artifacts). The approval response must reference them in batch_decisions.
    """
    ids: list[str] = []
    # collect ALL data parts from artifacts + status.message + history
    all_parts: list[dict] = []
    for art in result.get("artifacts", []) or []:
        all_parts.extend(art.get("parts", []) or [])
    msg = (result.get("status") or {}).get("message") or {}
    all_parts.extend(msg.get("parts", []) or [])
    for h in result.get("history", []) or []:
        all_parts.extend(h.get("parts", []) or [])
    ...
    for part in all_parts:
        ...
        if data.get("decision_type"):
            continue  # already a decision response, not a request
        # Pattern 1: data.args.originalFunctionCall.id (the common case)
        oid = ((data.get("args") or {}).get("originalFunctionCall") or {}).get("id")
        if oid and oid not in ids:
            ids.append(oid)
        # Pattern 2: data.originalFunctionCall.id
        oid = (data.get("originalFunctionCall") or {}).get("id")
        if oid and oid not in ids:
            ids.append(oid)
        # Pattern 3: data.id (a tool-call ID like "toolu_...")
        oid = data.get("id")
        if oid and str(oid).startswith("toolu") and oid not in ids:
            ids.append(oid)
        # Pattern 4: data.hitl_parts[].originalFunctionCall.id
        for hp in data.get("hitl_parts", []) or []:
            oid = (hp.get("originalFunctionCall") or {}).get("id")
            if oid and oid not in ids:
                ids.append(oid)
    return ids
...
class A2AClient:
    ...
    def send_approval(
        ...
        """Send a structured HITL approval/rejection — a DataPart, NOT free text.

        kagent's requireApproval gate accepts ONLY structured DataPart decisions
        (hitl.go ExtractDecisionFromMessage: 'no text keyword matching'). A free-text
        'Approve' message/send is ignored and the gate stays paused (the resume fails
        with 'tool_use ids without tool_result'). The message must carry:
          - decision_type: 'approve' | 'reject'
          - batch_decisions: {<toolCallId>: 'approve'|'reject'} keyed by the pending
            tool-call IDs extracted from the prior input-required response.
        """
        tool_ids = _extract_pending_tool_ids(prior.raw)
        data: dict = {"decision_type": decision}
        if tool_ids:
            data["batch_decisions"] = {tid: decision for tid in tool_ids}
        ...
        if prior.context_id:
            message["contextId"] = prior.context_id
        ...
        for attempt in range(3):
            ...
            raise A2AError(f"A2A approval failed after retries: {last_exc}")
        ...
        return A2AResponse(
            text=_text_from_result(result),
            state=state,
            context_id=result.get("contextId") or prior.context_id,
            task_id=result.get("id"),
            needs_input=state == "input-required",
            raw=result,
        )

    async def send_async_approval(
        self,
        prior: A2AResponse,
        decision: str = "approve",
        reason: str | None = None,
    ) -> A2AResponse:
        """Async wrapper for send_approval (use from async bridge handlers)."""
        import asyncio

        return await asyncio.to_thread(
            self.send_approval, prior, decision=decision, reason=reason
        )
