...
async def handle_message(update: Update, _ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if resp.needs_input:
        if resp.task_id:
            pending[resp.task_id] = (chat_id, resp.context_id)
        await update.message.reply_text(
...
async def handle_callback(update: Update, _ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    decision = "Approve — proceed with the proposed change." if choice == "approve" else "Reject — do not proceed."
    ...
    try:
        resp = await a2a.send_async(decision, context_id=cid)
    except A2AError as exc:
    ...
    await q.edit_message_text(f"{icon} {decision}\n\n{body}")
