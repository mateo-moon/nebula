...
HITL over Matrix is text-based (no portable inline keyboards via nio): when the orchestrator
returns `input-required`, the bot asks the room to reply approve/reject; the next matching
reply resumes the paused turn on the same context.
...
async def _handle(room_id: str, text: str, cid: str | None, prefix: str = "") -> None:
    ...
    if resp.needs_input:
        ...
        await _send(room_id, f"{prefix}{resp.text or '(no reply)'}")
...
async def _on_message(room: MatrixRoom, event: RoomMessageText) -> None:
    ...
    cid = contexts.get(room.room_id)
    low = body.lower()
    # HITL resume: a bare approve/reject in a room awaiting input continues the gated turn.
    if cid and low in ("approve", "reject", "yes", "no"):
        approved = low in ("approve", "yes")
        decision = "Approve — proceed." if approved else "Reject — do not proceed."
        await _handle(room.room_id, decision, cid, prefix=f"{'✅' if approved else '❌'} {decision}\n\n")
        return
    await _handle(room.room_id, body, cid)
