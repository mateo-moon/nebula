...
`X-Hub-Signature-256` HMAC, map the event to a prompt (`pull_request` opened/reopened,
...
gated behind propose-then-approval: when the orchestrator returns `input-required` we post
the approval question and stash the turn; a later `issue_comment` "approve"/"reject" on the
same issue/PR resumes it with a structured DataPart decision.
...
# Pending input-required approvals keyed by (owner, repo, number). The FULL A2AResponse is
# kept so send_async_approval can extract the pending tool-call IDs from resp.raw — kagent's
# requireApproval gate IGNORES free-text Approve/Reject, it needs a DataPart decision. H7:
# without this + the issue_comment handler below, the "reply approve/reject" prompt was a
# dead end and approvals never resumed.
pending: dict[tuple[str, str, int], A2AResponse] = {}
...
def _parse_comment(body: dict) -> tuple[str, str, int, str, str] | None:
    """Parse an `issue_comment` webhook into a HITL decision.

    Returns (owner, repo, number, decision, commenter) when a human (NOT the bot) posted a
    bare approve/reject on an issue/PR — the signal to resume a pending gated turn. Returns
    None for any other comment (edited/deleted comments, the bot's own posts, or non-keyword
    comments which we don't forward to the orchestrator).

    Loop guard: events from GITHUB_ACTOR are dropped so the bot doesn't react to the very
    "reply approve/reject" / decision comments it posts. Even if GITHUB_ACTOR is unset the
    exact-keyword match saves us: none of the bot's comment bodies is a bare "approve"/"reject".
    """
    if body.get("action") != "created":
        return None  # only a freshly-posted comment can resume the gate
    sender = body.get("sender", {}).get("login", "")
    if GITHUB_ACTOR and sender == GITHUB_ACTOR:
        return None  # loop guard: ignore the bot's own comments
    repo_obj = body.get("repository", {}) or {}
    ...
    if not owner or not name or number is None:
        return None
    # Exact match (after strip+lower) so "I don't approve of this" doesn't fire the gate.
    text = (body.get("comment", {}) or {}).get("body", "")
    text = (text or "").strip().lower()
    if text not in ("approve", "reject", "yes", "no"):
        return None
    decision = "approve" if text in ("approve", "yes") else "reject"
    return owner, name, int(number), decision, sender
...
async def github(
    ...
    # issue_comment carries no new prompt — it's the human's HITL decision on a pending
    # approval. Resume the gated turn with a structured DataPart (kagent's requireApproval
    # gate IGNORES free text). H7: this branch is what makes the "reply approve/reject"
    # prompt functional.
    if x_github_event == "issue_comment":
        parsed = _parse_comment(body)
        if parsed is None:
            return JSONResponse(content={"status": "ignored", "event": x_github_event})
        owner, name, number, decision, commenter = parsed
        key = (owner, name, number)
        prior_resp = pending.pop(key, None)
        if prior_resp is None:
            # Nothing pending on this issue/PR — leave the comment alone (post no noise).
            return JSONResponse(
                content={"status": "no_pending", "event": x_github_event, "decision": decision}
            )
        log.info("issue_comment %s/%s#%s by %s -> HITL %s", owner, name, number, commenter, decision)
        try:
            resp = await a2a.send_async_approval(prior_resp, decision=decision)
        except A2AError as exc:
            _post_comment(owner, name, number, f"⚠️ orchestrator error: {exc}")
            return JSONResponse(status_code=502, content={"status": "a2a_error", "error": str(exc)})
        icon = "✅" if decision == "approve" else "❌"
        _post_comment(owner, name, number, f"{icon} **{decision}** by @{commenter}\n\n{resp.text or '(done)'}")
        if resp.needs_input:
            # Another gate in the same turn — re-arm and ask again on this issue/PR.
            pending[key] = resp
            _post_comment(
                owner, name, number,
                f"🔐 Approval needed:\n\n{resp.text}\n\n"
        ...
        return JSONResponse(content={"status": "resolved", "event": x_github_event, "state": resp.state})
    ...
    if resp.needs_input:
        # Stash the FULL response so a later issue_comment approve/reject on this issue/PR
        # can call send_async_approval (which pulls pending tool-call IDs from resp.raw).
        pending[(owner, name, int(number))] = resp
